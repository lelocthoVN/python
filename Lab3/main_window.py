...
class MainWindow(QMainWindow):
    ...
    def initUI(self):
        ...
        anno = Annotation("dataCSV.csv")
        if not os.path.exists("dataCSV.csv"):
        ...
        next_dog_button.clicked.connect(lambda label="dog", iter=iter_ann: self.next_file("dog", iter_ann))
        next_dog_button.setStyleSheet("background-color : #FF99CC")

        next_cat_button = self.add_button("Следующий кот", 250, 50, 5, 250)
        next_cat_button.clicked.connect(lambda label="cat", iter=iter_ann: self.next_file("cat", iter_ann))
    ...
    def dataset_random(self) -> None:
        ...
        if ok:
            A = Annotation(f"{str(text)}.csv")
            dataset_random(self.folder_path, path_copy, A)
